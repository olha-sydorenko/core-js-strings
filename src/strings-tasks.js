...
function removeFirstOccurrences(str, value) {
  const startIndex = str.toLowerCase().indexOf(value.toLowerCase());
...
function removeLastOccurrences(str, value) {
  const startIndex = str.toLowerCase().lastIndexOf(value.toLowerCase());
  const endIndex = startIndex + value.length;
  if (startIndex >= 0)
    return str.substring(0, startIndex).concat(str.substring(endIndex));
  return str;
...
function isPalindrome(str) {
  const regex = /[\W_]/g;
  const originalString = str.toLowerCase().replaceAll(regex, '');
...
function findLongestWord(sentence) {
  const arr = sentence.replace(/[^a-zA-Z\s:]/g, '').split(' ');
  let longestWord = '';
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.charAt(i) === str.charAt(i).toUpperCase()) {
      invertedString += str.charAt(i).toLowerCase(char);
    } else {
      invertedString += str.charAt(i).toUpperCase(char);
    }
  }
